...
export class ImageData {
	...
	async get(index: GlTfId): Promise<HTMLImageElement> {
		...
		if (image.bufferView !== undefined) {
			...
			const bufferView = await this.asset.bufferViewData(
				image.bufferView
			);
			isObjectURL = true;
			...
			sourceURI = this.manager.resolveURL(
				resolveURL(image.uri, this.baseUri)
			);
		} else {
			/* istanbul ignore next */
			throw new Error(
	...
	async preFetchAll(): Promise<void[]> {
		...
		return Promise.all(images.map((_, i): any => this.get(i))) as Promise<
			void[]
		>;
	}
}
